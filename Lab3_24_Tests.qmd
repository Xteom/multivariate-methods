---
title: 'Lab3: Ejemplos y ejercicios de Pruebas de hipótesis'
...
Frets (1921) tomó medidas antropométricas del primer y segundo hijo en una muestra de 25 familias. Las variables que consideró son las siguientes (medidas en mm):
...
```{r}
library(MASS)
parcoord(frets[,c(1,2,3,4)], main = "Medidas de cabeza 25 pares de hijos", col = c("red","blue","red4","blue4"), lwd = c(1,1,4,4))
...
Para este ejercicio se puede hacer una prueba de hipótesis para cada media, cada varianza y cada covarianza. Con 4 variables se tienen 14 parámetros a probar y hay $2^{14} = 16,384$ posibles subconjuntos a formar combinando estos parámetros.

Los estimadores del vector de media y de matriz de varianzas y covarianzas son:
...
# También se puede usar la función mahalanobis: 
n*mahalanobis(xbar[c(1,3)], center = mu0, cov = S0)

# p-value de la distribución chi^2 con 2 grados de libertad
...
$$ 25(185.72 -\mu_1, 183.84-\mu_3)'0.01I_2(185.72-\mu_1,183.84-\mu*3)
< \chi^2_{0.95,2}$$

que es lo mismo que la región elíptica

$$ (185.72 -\mu_1)^2 + (183.84-\mu_3)^2 < 23.96 $$
...
# los puntos que están dentro de la elipse son los que no se rechazan
```
...
# S1 <- (n-1)/n*var(X) # Otra forma de calcular S1

(T2 <- (n-1)*(det(S0)/det(S1)-1))# Estadística de prueba
(F0 <- T2*(n-p)/(p*(n-1))) # sale de despejar las ecuación de al T^2
1-pf(F0,p,n-p) # p-value
```
...
Los siguientes datos corresponden a la transpiración de 20 mujeres sanas:

-   $V_1=$ tasa de duración
-   $V_2=$ contenido de sodio
-   $V_3=$ contenido de potasio

Se quiere probar la hipótesis $H_0: \mu = [4,50,10]'$ vs $H_1: \mu \neq [4,5,10]'$ a un nivel de significancia del 10%. Bajo el
...
(F0 <- (n-p)/(p*(n-1))*T2)
1 - pf(F0,p,n-p) # Se rechaza al 10%
```

También se puede comparar T2 y F0: Se rechaza $H_0$ si

$$ T^2 > \frac{(n-1)p}{n-p}F_{p,n-p}(\alpha) $$

```{r}
(n-1)*p/(n-p)*qf(0.10,p,n-p, lower.tail = F)
```

### Caso C

Esta es una prueba para varianzas. La estadística de prueba es de la forma:

$$ -2\log\lambda = n \cdot tr(\Sigma_0^{-1}S) - n\log(\Sigma_0^{-1}S) -np $$

Sea $a$ el promedio y $g$ la media geométrica de los valores propios de $\Sigma_0^{-1}S$. De tal manera que $tr(\Sigma_0^{-1}S)= pa$ y $|\Sigma_0^{-1}S| = g^p$ Entonces la prueba se puede escribir como:

$$-2\log\lambda = np(a-\log(g)-1)$$

La estadística no tiene una distribución simple. Entonces conviene usar la distribución asintótica de $-2\log\lambda$ que es  $\chi^2_{(p(p+1)/2)}$

En los datos de Frets, considerando por ejemplo la hipótesis $H_0: \Sigma = 100I_2$ tenemos que: 

```{r}
S0 <- 100*diag(2)
(Sn <- ((n-1)/n)*var(frets)) 
S1 <- Sn[c(1,3),c(1,3)] # Sn es el EMV de los datos
(lambdas <- eigen(solve(S0) %*% S1)$values)
a <- mean(lambdas) ; g <- exp(mean(log(lambdas)))
prueba <- n*p*(a - log(g) - 1)
prueba #valor de la estadística
pchisq(prueba, p*(p+1)/2, lower.tail = F) #p-value de la distribución asintótica
```

La hipótesis claramente se rechaza. Esto lo sabíamos desde el principio, ya que originalmente
los datos están altamente correlacionados (porque son hermanos). Podemos probar otra hipótesis: $H_0: \Sigma = \begin{pmatrix} 100 & 50 \\ 50 & 100\end{pmatrix}$.
...
Xi0 <- n*p*(a-log(g)-1) # Estadística
1-pchisq(Xi0,p*(p+1)/2) # No se rechaza la hipótesis. 
```
...
1.  Obtener los intervalos de confianza **simultáneos** del 95% para $\mu_1$, $\mu_2$ y $\mu_3$. (se den los dos al mismo tiempo)

```{r}
# y_a = a'x
c1 <- p*(n-1)/(n-p)*qf(.05,p,n-p,lower.tail = F)
# es como si obtuvieramos el intervalo de confianza para cada componente pero el cuantil 
# no es el de la normal, es el que cumple con la distribución F (distribución conjunta)
...
plot(Ti2, ylim = c(0,1.4*max(Ti2)), pch = 16, 
     main = expression(paste("gráfica estadística ", T^2)), 
     xlab = "observación",
