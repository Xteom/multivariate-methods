...
## Repaso matricial visto la última clase

Consideremos la matriz $X$ con las variables `DOB_MM`, `DOB_WK`, `APGAR5`, `UPREVIS`, `ESTGEST` y `DBWT`. Entonces $X$ tiene dimensiones $n=427323$ y $p=6$

```{r}
X <- births2006.smpl %>% select(DOB_MM, DOB_WK, APGAR5, UPREVIS, ESTGEST, DBWT)
```

Podemos calcular el vector de medias $\bar{{\bf x}}$, la matriz de varianzas y covarianzas ${\bf S}$ (sin sesgo) o ${\bf S}_n$ (con sesgo) y la matriz  de correlaciones ${\bf R}$.

```{r}
# Vector de medias:
n <- nrow(X)
uno <- function(n) {
            matrix(rep(1, n), nrow = n)  # vector de unos
         }
1 / n * t(X) %*% uno(n)  # forma algebraica. Tiene el problema de los valores NA.
(xbar <- colMeans(X, na.rm = TRUE))

# Matriz de covarianza.
(S <- var(X, na.rm = TRUE)) # da la matriz insesgada
(Sn <- (n - 1) / n * S) # no hay cambio significativo dado el tamaño de la muestra.

#H <- diag(n) - 1/n* uno(n) %*% t(uno(n)) # No se puede calcular la matriz de centrado por el tamaño. 

# Matriz de correlaciones Puede manejar los valores faltantes de muchas formas
(R <- cor(X, use = "na.or.complete"))
(D <- diag(diag(S)))
(solve(sqrt(D)) %*% S) %*% solve(sqrt(D))
```

Tenemos la ventaja de poder visualizar la matrix de correlaciones: 
...
Distancia de Mahalanobis respecto a S y la media de la distribución:

```{r}
m <- mahalanobis(X, center = xbar, cov = S)
head(m)
```

Como último ejemplo, podemos dividir la matriz de datos en dos conjuntos: un conjunto ${\bf w}$ con las primeras dos columnas o variables, y ${\bf v}$ con las cuatro columnas adicionales y podemos calcular las covarianzas entre ellas:

```{r}
w <- X %>%  select(1:2)
v <- X %>%  select(3:6)
cov(w, v, use = "na.or.complete")  # Extrae la covarianza entre las dos submatrices de datos
```
...
```{r}
library(psych)
describe(births2006.smpl, check = TRUE, na.rm = TRUE, omit = TRUE)  # También hay opción describe.by para grupos.
describeBy(births2006.smpl, group = "SEX") #No permite omitir las variables no numéricas.
```
...
p <- births2006.smpl %>%
        group_by(SEX) %>%
        select(DBWT, SEX) %>%
        ggplot(aes(x = DBWT / 1000, fill = SEX)) +
...
legend("topright", legend = c("M", "H"), col = c("pink3", "blue"), lwd = c(3,3))
title(main = "Distribución del peso de bebes\n por sexo", xlab = "peso (kg)", ylab = "Densidad")
```
...
ggplot(data =  births2006.smpl, aes(DBWT, geom = "density", fill = SEX)) +
       ...
       facet_grid(SEX ~ .) +
labs(title = "Histograma de nacimientos por género (en gramos)",
      y = "Conteo",
...
ggplot(data =  births2006.smpl, aes(DBWT, geom = "density", fill = SEX)) +
       geom_density() +
       facet_grid(SEX ~ .) +
labs(title = "Densidad por género (en gramos)",
      y = "Conteo",
      x = "Peso del bebé al nacer")
```
...
tabla
dotplot(tabla, groups = FALSE, main = "Distribución de los nacimientos por mes\ny día de la semana")
...
corrplot(cor(a[, -6], use = "complete.obs"), is.corr = TRUE,
         method = "ellipse", diag = TRUE,  addCoefasPercent = FALSE, tl.col = "blue", addCoef.col = "red")
```
